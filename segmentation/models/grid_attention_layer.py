...
class GridAttentionBlock3D(nn.Module):
    ...
    def _concatenation(self, x, g):
        ...
        sigm_psi_f = torch.sigmoid(self.psi(F.relu(theta_x + F.interpolate(self.phi(g), size=theta_x_size[2:],
                                                                           mode=self.upsample_mode, align_corners=True)
                                                   , inplace=True)))

        W_y = self.W(F.interpolate(sigm_psi_f, size=input_size[2:], mode=self.upsample_mode,
                                   align_corners=True).expand_as(x) * x)
